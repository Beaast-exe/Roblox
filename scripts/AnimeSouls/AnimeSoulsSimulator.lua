...
local Client = {
	...
	Suffixes = {
		"K", "M", "B", "T", "Qd", "Qn", "Sx", "Sp", "O", "N", "D",
		"Ud", "Dd", "Td", "Qdd", "Qnd", "Sxd", "Spd", "Od", "Nd",
		"V", "Uv", "Dv", "Tv", "Qdv", "Qnv", "Sxv", "Spv", "Ov", "Nv",
		"Tr", "Ut", "Dt", "Tt", "Qdt", "Qnt", "Sxt", "Spt", "Ot", "Nt"
	}
}
...
local Utilities = {} do
	function Utilities:GetEntityNames()
		...
		for _, entity in ipairs(Entities) do
			table.insert(EntityNames, entity.Player.Name)
		end
	...
	function Utilities:IsAlive(Player, StateCheck)
		...
		if table.find(self:GetEntityNames(), Player.Name) then
			local _, entity = Entity.getEntityFromPlayer(Player)

			return ((not StateCheck) or entity and entity.Humanoid:GetState() ~= Enum.HumanoidStateType.Dead) and entity
		else
	...
	function Utilities:GetEnemyWithName()
		...
		local Index, Position = GetNearestValue(LocalPlayer.Changed.HumanoidRootPart.Position, Positions)
		return Enemies[Index], Position
	end
...
local function GameNotify(Type, Message)
	task.spawn(function()
		pcall(function()
			...
			game:GetService("TweenService"):Create(Notification.Text, TweenInfo(0.2), {
...
local Window = Library:CreateWindow({
	Title = string.format("Beaast Hub | %s", GameName),
	Center = true,
	AutoShow = true
})
...
--SaveManager:SetFolder("BeaastHub/Anime Souls Simulator");
